...
export function PaymentModal({ form, orderFormat, onClose, discountPercent = 0 }: Props) {
  ...
  const userLabel = form.game === 'magicchessgogo' ? 'ID User' : 'USER ID';
  const serverLabel = form.game === 'magicchessgogo' ? 'Zone ID' : 'SERVER ID';
  const showServer = !['freefire', 'freefire_th'].includes(form.game);
  ...
  const verifyCurrentPrice = async () => {
    try {
      ...
      switch (form.game) {
        ...
        case 'magicchessgogo': tableName = 'magicchessgogo_products'; break;
  ...
  const storePayment = async (md5: string) => {
    try {
      ...
      const gameName = 
        ...
        form.game === 'freefire_th' ? 'Free Fire TH' :
        form.game === 'magicchessgogo' ? 'Magic Chess GoGo' :
        'Unknown Game';

      const orderMessage = `Top up successful✅\n\n- Transaction: ${txId}\n- Game: ${gameName}\n- Amount: ${finalAmount} $\n- Item: ${form.product?.name}\n- ${userLabel}: ${form.userId}\n- ${serverLabel}: ${form.game === 'freefire' || form.game === 'freefire_th' ? '0' : form.serverId}\n- Order ID: S${orderId}\n- Order Date: ${orderDate}`;
  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      if (data.responseCode === 0) {
        ...
        // Forward to group with order details
        const orderMessage = `Top up successful✅\n\n- Transaction: ${transactionId}\n- Game: ${
          ...
          form.game === 'magicchessgogo' ? 'Magic Chess GoGo' :
          'Unknown Game'
        }\n- Amount: ${finalAmount} $\n- Item: ${form.product?.name}\n- ${userLabel}: ${form.userId}\n- ${serverLabel}: ${form.game === 'freefire' || form.game === 'freefire_th' ? '0' : form.serverId}\n- Order ID: S${Date.now()}\n- Order Date: ${new Date().toLocaleString('km-KH', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false, timeZone: 'Asia/Phnom_Penh' })}`;
        const telegramUrl = `https://t.me/${storeConfig.supportGroup}?text=${encodeURIComponent(orderMessage)}`;
  ...
  const successReceipt = useMemo(() => (
    <div ref={receiptRef} className="bg-gray-50 rounded-lg p-4 space-y-4">
      ...
      <div className="space-y-3 text-gray-700 text-sm">
        ...
        <div className="flex justify-between border-b border-gray-200 pb-2">
          <h4 className="font-semibold">{userLabel}</h4>
          <p>{form.userId}</p>
        </div>
        {showServer && (
          <div className="flex justify-between border-b border-gray-200 pb-2">
            <h4 className="font-semibold">{serverLabel}</h4>
            <p>{form.serverId}</p>
          </div>
        )}
        {form.nickname && (
  ...
  ), [form.userId, form.serverId, form.nickname, form.game, productName, finalAmount, transactionId, handleContinueShopping, handleDownloadReceipt, userLabel, serverLabel, showServer]);
            ...
            <div className="space-y-4">
              <div className="rounded-lg p-4 space-y-3">
                {qrCode && !qrExpired ? (
                  ...
                  <div style={{ fontFamily: "SF_Pro_Display, 'SF Pro Display', Battambang", textAlign: 'center', boxSizing: 'border-box' }}>
