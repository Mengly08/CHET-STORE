...
export function AdminPanel({ onLogout }: AdminPanelProps) {
  ...
  const [magicchessgogoProducts, setMagicchessgogoProducts] = useState<GameProduct[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'dashboard' | 'mlbb' | 'mlbb_ph' | 'freefire' | 'magicchessgogo' | 'resellers' | 'prices' | 'promos'>('dashboard');
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const [
        ...
        { data: magicchessgogoData, error: magicchessgogoError }
      ] = await Promise.all([
        ...
        supabase.from('magicchessgogo_products').select('*').order('id', { ascending: true })
      ]);

      if (mlbbError) throw mlbbError;
      if (mlbbPhError) throw mlbbPhError;
      if (ffError) throw ffError;
      if (magicchessgogoError) throw magicchessgogoError;
      ...
      setMagicchessgogoProducts(magicchessgogoData.map(transformProduct('magicchessgogo')));
    } catch (error) {
  ...
  const transformProduct = (game: string) => (product: any): GameProduct => ({
    ...
    game: game as 'mlbb' | 'mlbb_ph' | 'freefire' | 'magicchessgogo',
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      const tableName = newProduct.game === 'mlbb' ? 'mlbb_products'
                     ...
                     : newProduct.game === 'freefire' ? 'freefire_products'
                     : 'magicchessgogo_products';
      ...
      const productData = {
        ...
        tagname: newProduct.tagname || null,
  ...
  const handleEditProduct = async (e: React.FormEvent) => {
    ...
    try {
      const tableName = editingProduct.game === 'mlbb' ? 'mlbb_products'
                     ...
                     : editingProduct.game === 'freefire' ? 'freefire_products'
                     : 'magicchessgogo_products';
      ...
      const productData = {
        ...
        tagname: editingProduct.tagname || null,
  ...
  const handleDeleteProduct = async (product: GameProduct) => {
    ...
    try {
      const tableName = product.game === 'mlbb' ? 'mlbb_products'
                       ...
                       : product.game === 'freefire' ? 'freefire_products'
                       : 'magicchessgogo_products';
  ...
  const getCurrentProducts = () => {
    switch (activeTab) {
      ...
      case 'magicchessgogo': return magicchessgogoProducts;
      default: return [];
    }
  };

  const getColumnCount = () => {
    return (activeTab === 'freefire' || activeTab === 'magicchessgogo') ? 7 : 8;
  };
        ...
        <div className="md:hidden fixed inset-0 z-40 bg-white/90 backdrop-blur">
          <div className="flex flex-col h-full p-4 pt-20">
            {[
              ...
              { id: 'magicchessgogo', label: 'Magic Chess GoGo', icon: <ShoppingBag className="w-5 h-5" /> },
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <div className="hidden md:block border-b border-gray-200">
            <nav className="flex -mb-px">
              {[
                ...
                { id: 'magicchessgogo', label: 'Magic Chess GoGo', icon: <ShoppingBag className="w-4 h-4" /> },
                ...
                <button
                  ...
                  onClick={() => {
                    setActiveTab(tab.id as any);
                    if (['mlbb', 'mlbb_ph', 'freefire', 'magicchessgogo'].includes(tab.id)) {
          ...
          <div className="md:hidden p-4 border-b border-gray-200">
            <div className="flex items-center gap-2">
              ...
              {activeTab === 'magicchessgogo' && <ShoppingBag className="w-5 h-5 text-blue-600" />}
              ...
              <h2 className="text-lg font-semibold text-gray-900">
                {activeTab === 'dashboard' ? 'Dashboard'
                 ...
                 : activeTab === 'magicchessgogo' ? 'Magic Chess GoGo'
          ...
          <div className="p-4 md:p-6">
            {activeTab === 'dashboard' ? (
              ...
              <ResellerPriceManager
                ...
                magicchessgogoProducts={magicchessgogoProducts}
              />
            ) : activeTab === 'promos' ? (
              <PromoCodeManager />
            ) : (
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                  <h2 className="text-lg md:text-xl font-semibold text-gray-900">
                    {activeTab === 'mlbb' ? 'Mobile Legends Products'
                     ...
                     : activeTab === 'freefire' ? 'Free Fire Products'
                     : 'Magic Chess GoGo Products'}
                  </h2>
                  <button
                    onClick={() => {
                      setShowAddForm(true);
